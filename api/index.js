...
// Allow all origins so your frontend doesn't get blocked
app.use(cors({
  origin: "*",
...
app.get("/api/tasks", async (req, res) => {
  try {
    ...
    const tasks = await db.collection("assignment").aggregate([
      ...
      { $addFields: { client: { $arrayElemAt: ["$clientData", 0] } } },
  ...
  try {
    ...
    if (clientId === 'new' && (clientName || clientPhone)) {
      const newClient = await db.collection("clients").insertOne({ 
        name: clientName || "Unnamed Client", phone: clientPhone || "", university: clientUniversity || "", createdAt: new Date(), updatedAt: new Date()
      });
    ...
    const task = await db.collection("assignment").insertOne({
      ...
      deadline: deadline || null, totalValue: Number(totalValue) || 0, advancePaid: Number(advancePaid) || 0, 
      assignedTo, status: 'pending', createdAt: new Date(), updatedAt: new Date()
    });
  ...
  try {
    ...
    if (finalClientId === 'new') {
      if (updates.clientName || updates.clientPhone) {
        const newClient = await db.collection("clients").insertOne({ 
          name: updates.clientName || "Unnamed Client", phone: updates.clientPhone || "", university: updates.clientUniversity || "", createdAt: new Date(), updatedAt: new Date()
        });
        finalClientId = newClient.insertedId;
...
app.get("/api/accounts", async (req, res) => {
  ...
  try {
    const client = await clientPromise;
    const db = client.db("ak_process");
    const data = await db.collection("accounts").find({}).sort({ date: -1 }).toArray();
    cache.accounts = data;
    res.json(data);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
